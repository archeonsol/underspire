...
def node_start(caller, raw_string, **kwargs):
    ...
    options = [
        {"desc": "Return to a body (play)", "goto": "node_select_character"},
        {"desc": "Forge a new soul (create character)", "goto": "node_create_character"},
        {"desc": "Read the directives", "goto": "node_rules"},
        {"desc": "Sign out", "goto": "node_quit"},
    ]
    return text, options


def node_select_character(caller, raw_string, **kwargs):
    """List the account's characters (one per account; after go light this is empty)."""
    ...
    options = []
    for char in chars:
        options.append({"desc": char.key, "goto": (_puppet_character, {"char": char})})
    options.append({"desc": "Back to Soul Registry", "goto": "node_start"})
    return text, options


def _puppet_character(caller, raw_string, **kwargs):
    """Puppet the account into the chosen character and close the menu."""
    ...
    try:
        ...
        return "node_start"
    return None  # Menu already closed
...
def start_main_menu(caller):
    ...
    EvMenu(caller, "world.main_menu", startnode="node_start")
