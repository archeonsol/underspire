    ...
    def func(self):
        ...
        if verb in ("install", "remove", "replace", "repair", "list"):
            ...
            if verb == "list":
                ...
                target = patient
                if target_name:
                    looked = caller.search(target_name, location=caller.location)
                    if looked:
                        target = looked
                ...
                if not cyber:
                    ...
                    caller.msg("\n".join([f"Installed on {target.key}:"] + [f"  {c.key} (#{c.id})" for c in cyber]))
                return
            if verb == "install":
                # surgery install <cyberware> on <patient>
                ...
                target = caller.search(right.strip(), location=caller.location)
